...
/**
 * MCP tool parameters for managing filters
 */
interface ManageFiltersParams {
  action: 'read' | 'update';
  keyword?: string;
 ...
 */
export async function handleLinkedInManageFilters(params: ManageFiltersParams): Promise<{
  ...
  try {
    ...
    if (params.action === 'update') {
      ...
      if (Object.keys(updates).length === 0) {
        return {
          content: [{
            type: "text",
            text: `⚠️ No filter updates provided. Use action: 'read' to view current filters, or provide filter parameters to update.`
          }]
        };
      }
    ...
    return {
      content: [{
        type: "text",
        text: `❌ Failed to manage filters: ${error instanceof Error ? error.message : 'Unknown error'}`
      }]
    };
  }
}
